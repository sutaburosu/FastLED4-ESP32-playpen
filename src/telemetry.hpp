...
struct Telemetry {
    ...
    void sendUDP(uint32_t minMs = 200) {
        ...
        if (!flags.wifiConnected || !flags.udpTelemetry ||
            ...
            // Logging starts before WiFi connects. Queue startup stuff until
            // connect
            while (udpQueue.size() > 50)
                udpQueue.pop_back(); // drop newest entries, to preserve boot
                                     // messages
            return;
        }

        // Cap at 5 packets per second, to aid coalescing and reduce header
        // overhead
        if (!udpSendMs)
            udpSendMs = millis();
        ...
        size_t len =
            sendto(udpSocket, pkt_cstr, telemetryPacket.length(), 0,
                   (struct sockaddr *)&udpSockAddr, sizeof(udpSockAddr));
        if (len <= 0)
            return;

        if (true) { // send Telemetry bandwidth stats?
            ...
            if (elapsed >= 5000) {
                add("UDP bandwidth",
                    {.value = String(udpBytes * 1000.f / elapsed),
                     .unit = "B/s"});
                add("UDP packets",
                    {.value = String(udpPackets * 1000.f / elapsed),
                     .unit = "pkts/s"});
                add("UDP avg size",
    ...
    void begin() {
        ...
        add("Heap Max",
            {.maxMs = 5000}); // problems stacking graphs in Teleplot
