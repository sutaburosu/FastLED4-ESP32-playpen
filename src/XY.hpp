...
template <int config, uint16_t width, uint16_t height, uint16_t panel_width,
          uint16_t panel_height>
uint16_t XY_panels(uint16_t x, uint16_t y, uint16_t w = 0, uint16_t h = 0) {
    (void)w, (void)h;

    // if (x >= width || y >= height)
    //   return -1; // the caller must detect results >= NUM_LEDS and not plot
    //   them
    ...
    if (false) // slower like this, so less DRY it must be
        return (panel_offset +
                XY_panel<config, panel_width, panel_height>(x, y));
...
#if defined(PANEL_WIDTH) && defined(PANEL_HEIGHT) &&                           \
    ((PANEL_WIDTH != MATRIX_WIDTH) || (PANEL_HEIGHT != MATRIX_HEIGHT))
XYMap xyMap = XYMap::constructWithUserFunction(
    MATRIX_WIDTH, MATRIX_HEIGHT,
    XY_panels<XY_CONFIG, MATRIX_WIDTH, MATRIX_HEIGHT, PANEL_WIDTH,
              PANEL_HEIGHT>);
#else
XYMap xyMap = XYMap::constructWithUserFunction(
    MATRIX_WIDTH, MATRIX_HEIGHT,
...
void benchmarkXYmaps() {
    ...
    XYMap xyMapPanel =
        XYMap::constructWithUserFunction(64, 64, XY_panel<XY_CONFIG>);
    us = micros(), sum = 0;
    for (uint32_t i = 0; i < iterations; i++)
        for (uint16_t y = 0; y < 64; y++)
            for (uint16_t x = 0; x < 64; x++)
    ...
    XYMap xyMapPanel_wh =
        XYMap::constructWithUserFunction(64, 64, XY_panel<XY_CONFIG, 64, 64>);
    us = micros(), sum = 0;
    for (uint32_t i = 0; i < iterations; i++)
        for (uint16_t y = 0; y < 64; y++)
            for (uint16_t x = 0; x < 64; x++)
    ...
    XYMap xyMapPanel_const = XYMap::constructWithUserFunction(
        64, 64, XY_panel_const<XY_CONFIG, 64, 64>);
    us = micros(), sum = 0;
    for (uint32_t i = 0; i < iterations; i++)
        for (uint16_t y = 0; y < 64; y++)
            for (uint16_t x = 0; x < 64; x++)
    ...
    XYMap xyMapPanels =
        XYMap::constructWithUserFunction(64, 64, XY_panels<XY_CONFIG, 32, 32>);
    us = micros(), sum = 0;
    for (uint32_t i = 0; i < iterations; i++)
        for (uint16_t y = 0; y < 64; y++)
            for (uint16_t x = 0; x < 64; x++)
    ...
    XYMap xyMapPanels_wh = XYMap::constructWithUserFunction(
        64, 64, XY_panels<XY_CONFIG, 64, 64, 32, 32>);
    us = micros(), sum = 0;
    for (uint32_t i = 0; i < iterations; i++)
        for (uint16_t y = 0; y < 64; y++)
            for (uint16_t x = 0; x < 64; x++)
